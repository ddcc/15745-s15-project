...
\paragraph{Stack accesses within frame} 
...
Experimental setup consisted of running our plugins with BAP \texttt{0.9.6}
\cite{bap} on a set of binaries. 
...
We observe from Figure~\ref{corestats} that this could number in the hundreds,
whereas the total number could range in the thousands (Figure ~\ref{coretotal}.
...
choose a test suite which purposefully exhibits the stack properties that we check;
see \S\ref{lessonslearned} for further discussion.
...
We were pleased with our ability to rise to the challenge of...
...
Neither of our checks were flagged as violations in the coreutils suite--
thus we have security outcomes, but it's not that 'great'. 

which functions?

A critical part of this project was developing an intuition for...
